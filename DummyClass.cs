...
namespace BoringRPG {
  internal class target : Archetype, ICanUseSkill {

    //пример для работы со случайными числами
    private static Random random = new Random();
    public bool LastHitWasCrit;

    public target(string name, int hp, int mp, int ammo, int dmg, double crit) : base(name, 100, 50, 10, 20, 0.3) {
      // HP=100, MP=50, Ammo=10, Damage=20, CritChance=0.3 (30%)
    }

    public target(string name) : base(name, 100, 50, 10, 20, 0.3) {
    }
