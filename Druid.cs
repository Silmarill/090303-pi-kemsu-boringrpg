...
namespace BoringRPG
{
    internal class Druid : Archetype, ICanUseSkill
    {
        ...
        {
            Console.WriteLine($"\n=== {Name} использует навык: {skill.Name} ===");

            // Сохраняем состояние до применения
            string beforeState = $"До применения: {Name} (HP: {HP}), {target.Name} (HP: {target.HP})";

            // Применяем навык
            skill.Use(this, target);

            // Выводим результат
            Console.WriteLine(beforeState);

            if (skill is SoulLink)
            {
                Console.WriteLine($"Души связаны! HP выровнены до {HP}");
            }

            Console.WriteLine($"После применения: {Name} (HP: {HP}), {target.Name} (HP: {target.HP})\n");
        }
        ...
        public static Druid operator +(Druid druid, MoonBerry berry)
        {
            ...
            Console.WriteLine($"{druid.Name} съел лунную ягоду! Шанс крита увеличен на {berry.Value}%!");
            return druid;
        }
